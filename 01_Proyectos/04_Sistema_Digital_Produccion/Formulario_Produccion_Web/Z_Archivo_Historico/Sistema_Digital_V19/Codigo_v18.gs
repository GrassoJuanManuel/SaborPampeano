// Código Backend (Google Apps Script) - VERSIÓN 19
  ...
  try {
    ...
    if (datos.produccion && datos.produccion.length > 0) {
      datos.produccion.forEach(function(p) {
        ...
        // La observación ya no se pone acá, va en su propia fila
    ...
    if (filasRegistros.length > 0) {
      ...
      // --- COLOR POR TURNO en REGISTROS (col Turno = 3) ---
      var colTurnoReg = 3; // Columna C = Turno
      for (var tr = 0; tr < filasRegistros.length; tr++) {
        ...
        if (colorFondo) {
          hoja.getRange(2 + tr, 1, 1, filasRegistros[0].length).setBackground(colorFondo);
        }
      }
    }
    ...
    if (entradas_ctrl.length > 0) {
      ...
      if (cCtrl.turno > 0) {
        for (var tc = 0; tc < filasCtrl.length; tc++) {
          ...
          if (colorCtrl) {
            hojaCtrl.getRange(2 + tc, 1, 1, ctrlLastCol).setBackground(colorCtrl);
          }
        }
      }
    }
