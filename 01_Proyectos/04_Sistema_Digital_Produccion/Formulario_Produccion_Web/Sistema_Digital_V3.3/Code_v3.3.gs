...
function onOpen() {
  ...
  ui.createMenu('📝 Formulario')
    ...
    .addItem('Actualizar Link del Reporte', 'actualizarLink')
...
function mostrarLink() {
  ...
  if (urlGuardada) {
    ...
    SpreadsheetApp.getUi().alert("Todavía no hay un link guardado. Usá la opción 'Actualizar Link del Reporte'.");
  }
}
...
function actualizarLink() {
  ...
  var respuesta = ui.prompt(
    "Actualizar Link del Formulario",
    "Pegá acá el NUEVO link largo que te generó la Nueva Implementación (el que termina en /exec):",
    ui.ButtonSet.OK_CANCEL
  );
  if (respuesta.getSelectedButton() == ui.Button.OK) {
    var nuevoLink = respuesta.getResponseText().trim();
    if (nuevoLink) {
      PropertiesService.getDocumentProperties().setProperty('WEB_APP_URL', nuevoLink);
      ui.alert("¡Éxito! El link se actualizó correctamente.");
    }
  }
}
function mostrarLinkEspecial(vista, titulo) {
  var urlGuardada = PropertiesService.getDocumentProperties().getProperty('WEB_APP_URL');
  if (!urlGuardada) {
    SpreadsheetApp.getUi().alert("Primero debés actualizar el link del reporte principal desde el menú '🌐 Actualizar Link del Reporte'.");
    return;
  }
  
  var linkFinal = urlGuardada + "?vista=" + vista;
  
  var html = '<div style="font-family:Arial,sans-serif;padding:15px;text-align:center;">'
    +'<h3 style="color:#4A5D23;margin-top:0;">' + titulo + '</h3>'
    +'<p style="font-size:13px;color:#555;">Este link se autocompletó a partir de tu link principal.</p>'
    +'<input type="text" id="urlInput" value="' + linkFinal + '" readonly style="width:100%;padding:10px;margin:15px 0;border:1px solid #ccc;text-align:center;font-size:14px;box-sizing:border-box;">'
    +'<button onclick="copiarLink()" style="background:#7A9B3A;color:#fff;border:none;padding:10px 20px;cursor:pointer;width:100%;font-weight:bold;border-radius:4px;">Copiar Enlace</button>'
    +'<script>function copiarLink(){var input = document.getElementById("urlInput"); input.select(); document.execCommand("copy"); input.value="¡Copiado!"; setTimeout(function(){input.value="' + linkFinal + '";},1500);}<\/script>'
    +'</div>';
    
  SpreadsheetApp.getUi().showModalDialog(HtmlService.createHtmlOutput(html).setWidth(400).setHeight(220), titulo);
}

function mostrarLinkLotes() {
  mostrarLinkEspecial('lotes', 'Link Tabla de Lotes');
}

function mostrarLinkStock() {
  mostrarLinkEspecial('stock', 'Link Control de Stock');
}
