// Código Backend (Google Apps Script) - VERSIÓN 2.3 (Con Parche Anti-Fallos de Sistema)
...
function doGet(e) {
  ...
  var htmlOutput;
  try {
    // 1. Intenta abrir el archivo con nombre normal
    htmlOutput = HtmlService.createHtmlOutputFromFile(archivo);
  } catch (error1) {
    try {
      // 2. Si el sistema de Google tiene el bug del ".html" explícito, intenta con esa extensión
      htmlOutput = HtmlService.createHtmlOutputFromFile(archivo + ".html");
    } catch (error2) {
      // 3. Fallback absoluto para que no colapse con el error de Google
