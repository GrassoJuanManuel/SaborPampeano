// Código Backend (Google Apps Script) - VERSIÓN 3.1 (Con Auto-completado de Lotes Mágico)
...
function autocompletarLotesMagico() {
  ...
  if (colCat === -1 || colVar === -1) {
    SpreadsheetApp.getUi().alert("Faltan las columnas 'Cat' o 'Var'. Creálas primero.");
    return;
  }
  ...
  for (var i = 1; i < datos.length; i++) {
    ...
    var res = mapearLoteMago(pName, pDesc);
...
// El algoritmo inteligente que detecta abreviaturas de Sabor Pampeano
function mapearLoteMago(producto, desc) {
  var txt = (producto + " " + desc).toUpperCase();
  ...
  if (txt.indexOf("BABAGANUSH") > -1) {
    if (txt.indexOf("BERENJENA X") > -1 || txt.indexOf("BERENJENA/PIMIENTOS") > -1) return [2, 6];
  ...
  if (txt.indexOf("TARTA") > -1) {
    ...
    if (txt.indexOf("JAMÓN Y QUESO") > -1) return [7, 3];
  ...
  if (txt.indexOf("SUPREMA DE POLLO C/ PAPAS") > -1 || txt.indexOf("SUPREMA DE POLLO C/PAPAS") > -1) return [8, 2];
  if (txt.indexOf("SUPREMA DE POLLO C/VERDURAS") > -1) return [8, 3];
